...
let rec fold f t acc =
  ...
  | Leaf -> acc
  | Node (_, l, v, r) ->
    let fold_left = fold f l acc in
    let fold_right = fold f r fold_left in
    f v fold_right
...
let rec partition f t=
  ...
  | Leaf -> (empty, empty)
  | Node (_, l, v, r) ->
    let val_left = partition f l in
    let val_right = partition f r in
    let new_val_left = union (fst val_left) (fst val_right) in
    let new_val_right = union (snd val_left) (snd val_right) in
    if f v then
      (add v new_val_left, new_val_right)
    else
      (new_val_left, add v new_val_right)


let rec subset t1 t2 =
  if is_empty (diff t1 t2) then
    true
  else 
    false
...
let ele t =
  let rec element_tries t acc=
    match t with
    | Leaf -> acc
    | Node (_, l, v, r) ->
      let list_right = element_tries r acc in
      let new_list = v::list_right in
      element_tries l new_list
    in
  element_tries t []
